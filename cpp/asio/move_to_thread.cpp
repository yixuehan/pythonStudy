...
public:
    ...
    ~TaskQueue()
    {
        cout << "is running:" << !_io_context.stopped() << endl;
        cout << "owns_work:" << _work_guard.owns_work() << endl;
        // if (!_work_guard.owns_work()) {
        //     asio::steady_timer timer(_io_context, 2s);
        //     timer.async_wait([this](const boost::system::error_code &){
        //             cout << "stop io_context\n";
        //             _io_context.stop();
        //             });
        // }
        _work_guard.reset();
        // _io_context.get_executor().running_in_this_thread
        cout << "owns_work:" << _work_guard.owns_work() << endl;
        if (_thread.joinable()) {
            _thread.join();
        }
